-- Create applications table
  ...
  -- Updated assessments table (add new columns)
  CREATE TABLE IF NOT EXISTS isl_assessments (
      id BIGINT AUTO_INCREMENT PRIMARY KEY,
      step_order INT,
      step_name VARCHAR(255),
      mode VARCHAR(100),
      passing_criteria INT, -- Changed to INT for complete numbers
      type VARCHAR(100),
      title VARCHAR(255),
      interviewerName VARCHAR(255), -- Updated without underscore
      sendFeedback BOOLEAN DEFAULT FALSE -- Updated without underscore
  );

  -- Create junction table for jobs and assessments
  CREATE TABLE IF NOT EXISTS isl_job_assessments (
